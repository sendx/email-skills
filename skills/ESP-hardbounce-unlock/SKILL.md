...
description: Hard bounce unlock request evaluation for ESP accounts. Use when an ESP user submits an unlock request after their account was locked for exceeding the hard bounce threshold (500+ hard bounces within 10,000 sends). Covers risk scoring, verification steps, decision logic, and customer response drafting.
metadata:
  version: 1.0.0
...
## Context
A SendX sub-account is automatically locked when it generates 500+ hard bounces within 10,000 emails sent.
The lock is permanent until manually unlocked. Users submit a form which creates an Intercom ticket.
Support reps monitor #sendpost-block-heartbeat in case someone is locked but never fills in the form.
...
Check via ClickHouse and Intercom:
...
2. Unlock after hygiene complete using: /sendx unlock <email> <team_id> <reason>
...
## Documentation
Write decision and reasoning in the Intercom ticket notes.
Significant cases documented to Drive:
  node /home/openclaw/.openclaw/workspace/drive-publish.js loop-support compliance-ACCOUNT-DATE.md content
