...
3. **Choose the right trigger**: Is this a SendX native event (tag added, drip clicked, form submitted)? Or is it from an integration (Shopify cart abandoned, Stripe charge succeeded, CRM contact created)? Pick the trigger that matches what actually happens in your business.
